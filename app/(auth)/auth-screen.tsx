...
import { Mail, Phone, Lock, User, ArrowRight, Sparkles, X } from "lucide-react-native";
...
export default function AuthScreen() {
    ...
    }, [step]); // Dependency array: runs whenever 'step' changes
    ...
    const handleLogin = async () => {
        ...
        Keyboard.dismiss();
        try {
            if (step === 'login' && mode === 'email') {
                await login(email, pass);
            } else if (step === 'signup') {
                console.log(`Simulating signup for ${mode}: ${mode === 'email' ? email : phone}, Name: ${name}`);
            } else if (step === 'login' && mode === 'phone') {
                console.log(`Simulating phone login for phone: ${phone}`);
            }
            router.push("/(protected)/(shop)");
        } catch (err: any) {
            console.log("Auth failed", err);
            setError(err.message || (step === 'signup' ? "Account creation failed" : "Login failed"));
        } finally {
            setLoading(false);
        }
    };
                            ...
                            transform: [
                                {
                                    scale: stepAnim.interpolate({
                                        inputRange: [0, 1],
                                        outputRange: [0.95, 1], // slight pop effect
                                    }),
                                },
                            ],
                        }}
